...
const projects = [
    {
        ...
        description: "A full-stack MERN marketplace connecting brands with creators. Real-time messaging via Socket.io, secure Stripe payments, RBAC + JWT, and Redux-powered dashboards.",
    ...
    },
    {
        ...
        description: "An editorial e-commerce experience for a premium fashion label. Cinematic dark UI, refined typography, and a curated lookbook flow built for conversions.",
    ...
    },
    {
        ...
        description: "Custom workouts, macro-aware diet plans, BMI & calorie tools — packaged in a sleek dark UI that helps users actually stick with their goals.",
    ...
    },
    {
        ...
        description: "The world's first Web3 VR English learning game for grades 3–12. 120 episodes, on-chain rewards, and immersive multiplayer adventures.",
...
function Card({ p, i }) {
    return (<motion.a href={p.url} target="_blank" rel="noreferrer" initial={{ opacity: 0, y: 60 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1], delay: (i % 2) * 0.15 }} className="group block">
      <div className="relative overflow-hidden rounded-2xl border border-border/60 bg-surface aspect-[16/10] mb-6">
        <img src={p.image} alt={p.title} loading="lazy" className="absolute inset-0 h-full w-full object-cover object-top transition-transform duration-[1.2s] ease-out group-hover:scale-105"/>
      ...
      <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
        ...
        <div className="flex flex-wrap gap-2">
          {p.stack.map((s) => (<span key={s} className="px-2.5 py-1 rounded-full border border-border/60 text-xs text-muted-foreground">
              {s}
            </span>))}
        </div>
      </div>
...
export function Work() {
    return (<section id="work" className="px-6 md:px-10 py-24 md:py-32">
